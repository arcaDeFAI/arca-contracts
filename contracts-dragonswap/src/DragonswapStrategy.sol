 ...
 */
contract DragonswapStrategy is Clone, ReentrancyGuardUpgradeable, IDragonswapStrategy {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeCast for uint256;
    using Math for uint256;
    using Uint256x256Math for uint256;

    // Additional errors not in IStrategyCommon but needed for Dragonswap
    error Strategy__ActiveIdSlippage();

    // ============ Dragonswap-Specific Events (NOT in interfaces) ============
    // Position Management
    ...
    {
        return IVaultFactory.StrategyType.Dragonswap;
    }
    ...
    function setOperator(address operator) external onlyFactory {
        require(operator != address(0), "DragonswapStrategy: zero address");
        _operator = operator;
        ...
        returns (uint128 liquidity, uint128 tokensOwed0, uint128 tokensOwed1)
    {
        INonfungiblePositionManager npm = INonfungiblePositionManager(
            _factory.getDragonswapNonfungiblePositionManager()
        );
    ...
    // ============ IDragonswapStrategy Implementation ============
    ...
    function getRewardTokens() external view returns (address[] memory) {
        ...
        address voterAddress = _factory.getDragonswapVoter();
    ...
    function hasExtraRewards() external pure returns (bool) {
        return false; // Dragonswap doesn't have extra rewards
    }
    ...
    function withdrawAll() external onlyVault {
        ...
        uint256 queuedShares = IOracleRewardDragonswapVault(vault)
            .getCurrentTotalQueuedWithdrawal();

        // Calculate queued amounts (if any)
        uint256 totalSupply = IOracleRewardDragonswapVault(vault).totalSupply();
    ...
    /**
     * @notice Dragonswap-specific rebalance, defensive version
        ...
        )
    {
        require(
            _positionTokenId == 0,
            "DragonswapStrategy: position already exists"
        );
    ...
    /**
     * @notice Calculate Dragonswap position value
     */
        ...
        )
    {
        ...
        queuedShares = IOracleRewardDragonswapVault(_vault())
            .getCurrentTotalQueuedWithdrawal();
        ...
        uint256 totalSupply = IOracleRewardDragonswapVault(_vault()).totalSupply();
        if (totalSupply > 0 && queuedShares > 0) {
    ...
    function _transferAndExecuteQueuedAmounts(
        ...
        if (queuedShares > 0) {
            ...
            IOracleRewardDragonswapVault(vault).executeQueuedWithdrawals();
        }
    }

    /**
     * @dev Harvests the rewards from the Dragonswap position NFT via the gauge system.
    ...
    function _getGaugeAddressSafely() internal view returns (address) {
        try _factory.getDragonswapVoter() returns (address voterAddress) {
     ...
     * @return gaugeAddress The gauge address
        ...
        )
    {
        ...
        gaugeAddress = _getGaugeAddressSafely();

        if (!hasActivePosition || gaugeAddress == address(0)) {
            return (
                new address[](0),
                new uint256[](0),
                gaugeAddress,
