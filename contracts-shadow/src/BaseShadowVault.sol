...
import {Clone} from "../../contracts-metropolis/lib/joe-v2/src/libraries/Clone.sol";
...
import {SafeCast} from "../../contracts-metropolis/lib/joe-v2/src/libraries/math/SafeCast.sol";
...
/**
 * @title Shadow Base Vault contract
 * @author Arca
 * @notice This contract is used to interact with Shadow (Ramses V3) pools. It should be inherited by a Vault
 ...
 */
abstract contract BaseShadowVault is Clone, ERC20Upgradeable, ReentrancyGuardUpgradeable, IBaseVault {
    ...
    /// @notice flag to indicate if the vault is flagged for shutdown
    bool private _flaggedForShutdown;

    /// @notice mapping to track user deposits timestamp
    ...
    /**
     * @dev Returns the address of the pool as ILBPair for interface compatibility.
     * @return The address of the pool cast as ILBPair.
     */
    function getPair() public pure virtual override returns (address) {
        return address(_pool());
    }

    /**
     * @dev Returns the address of the token X (token0 in Shadow).
    ...
    /**
     * @dev Returns the address of the token Y (token1 in Shadow).
    ...
    /**
     * @dev Returns the total balances of the vault.
     ...
     */
    function setStrategy(IStrategy newStrategy) public virtual override onlyFactory {
        ...
        // Verify that the strategy is valid, i.e. it is for this vault and for the correct pool and tokens.
        if (
            newStrategy.getVault() != address(this) || newStrategy.getPair() != address(_pool())
     ...
     * amount of tokens deposited in the pool.
