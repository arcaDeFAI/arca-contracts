...
contract MetroShadowStaking is ReentrancyGuard, Ownable, Pausable {
    ...
    // Events
    event MetroStaked(address indexed user, uint256 amount, uint256 nMetroMinted, uint256 tierId, uint256 positionIndex);
    event X33Staked(address indexed user, uint256 amount, uint256 nShadowMinted, uint256 positionIndex);
    event EarlyExit(address indexed user, uint256 positionIndex, uint256 amountReceived, uint256 penalty);
    event RegularUnlock(address indexed user, uint256 positionIndex, uint256 amount);
    event X33Unstaked(address indexed user, uint256 positionIndex, uint256 amount);
     ...
     */
    function stakeMetro(uint256 amount, uint256 tierId) external nonReentrant whenNotPaused {
        require(tierId <= 2, "Invalid tier ID");
        require(amount > 0, "Amount must be greater than 0");
        
        StakingTier memory tier = stakingTiers[tierId];
        require(METRO_TOKEN.transferFrom(msg.sender, address(this), amount), "Transfer failed");
        ...
        emit MetroStaked(msg.sender, amount, nMetroToMint, tierId, positionIndex);
    }
    ...
    function emergencyExit(uint256 positionIndex) external nonReentrant {
        require(positionIndex < userMetroPositions[msg.sender].length, "Invalid position");
        MetroStakePosition storage position = userMetroPositions[msg.sender][positionIndex];
        require(position.isActive, "Position not active");
        require(block.timestamp < position.unlockTime, "Position already unlocked");
        ...
        uint256 penaltyAmount = (position.amount * currentPenalty) / BASIS_POINTS;
        ...
        require(userReceives <= availableInPiggyBank, "Insufficient piggy bank funds");
        ...
        uint256 toStakers = (penaltyAmount * STAKER_REWARD_SHARE) / BASIS_POINTS;
        ...
        require(METRO_TOKEN.transfer(msg.sender, userReceives), "User transfer failed");
        ...
        if (totalPendingRewards >= totalNMetroMinted / 100) { // Trigger when rewards >= 1% of total supply
    ...
    function unlockMetro(uint256 positionIndex) external nonReentrant {
        ...
        uint256 totalToReturn = position.amount + position.rebaseRewardsReceived;
        require(METRO_TOKEN.transfer(msg.sender, totalToReturn), "Transfer failed");
    ...
    function _distributeRebaseRewards() internal {
        ...
        uint256 rewardsPerTokenIncrease = (totalPendingRewards * 1e18) / totalNMetroMinted;
    ...
    function _claimRebaseRewards(address user) internal {
        ...
        if (rewardsSinceLastClaim > 0) {
            ...
            for (uint256 i = 0; i < positions.length; i++) {
                if (positions[i].isActive) {
                    uint256 positionReward = (rewardsSinceLastClaim * positions[i].nMetroAmount) / userTotalNMetro;
    ...
    function stakeX33(uint256 amount) external nonReentrant whenNotPaused {
        require(amount > 0, "Amount must be greater than 0");
        require(X33_TOKEN.transferFrom(msg.sender, address(this), amount), "Transfer failed");
    ...
    function unstakeX33(uint256 positionIndex) external nonReentrant {
        require(positionIndex < userX33Positions[msg.sender].length, "Invalid position");
        X33StakePosition storage position = userX33Positions[msg.sender][positionIndex];
        require(position.isActive, "Position not active");
        ...
        // Return X33 tokens
        require(X33_TOKEN.transfer(msg.sender, position.amount), "Transfer failed");
     ...
     */
    function getUserMetroPositions(address user) external view returns (MetroStakePosition[] memory) {
     ...
     */
    function getUserX33Positions(address user) external view returns (X33StakePosition[] memory) {
     ...
     */
    function calculateCurrentPenalty(address user, uint256 positionIndex) external view returns (uint256) {
        require(positionIndex < userMetroPositions[user].length, "Invalid position");
        MetroStakePosition memory position = userMetroPositions[user][positionIndex];
     ...
     */
    function getPendingRebaseRewards(address user) external view returns (uint256) {
     ...
     */
    function updateStakingTier(uint256 tierId, uint256 multiplier, uint256 lockPeriod, uint256 penaltyRate) external onlyOwner {
        require(tierId <= 2, "Invalid tier ID");
