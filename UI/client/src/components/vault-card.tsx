...
export default function VaultCard({ vault, onClick }: VaultCardProps) {
  ...
  const [positionStatus, setPositionStatus] = useState<"Active" | "Inactive">(
    "Inactive",
  );
  const [approvalStep, setApprovalStep] = useState<
    "none" | "approving" | "depositing"
  >("none");
  ...
  const handleProvideLiquidity = async () => {
    ...
    try {
      ...
      const amountX =
        parseFloat(token0Amount) > 0 ? parseUnits(token0Amount, 18) : 0n;
      const amountY =
        parseFloat(token1Amount) > 0 ? parseUnits(token1Amount, 6) : 0n;
  ...
  const handlePercentageClick = (token: string, percentage: number) => {
    if (activeTab === "deposit") {
      const balance =
        token === "S" ? tokenXBalance.balance : tokenYBalance.balance;
      ...
      const sharesBalance = userVaultShares
        ? Number(userVaultShares) / 1e12
        : 0;
  ...
  return (
    <>
              ...
              <div className="bg-arca-surface rounded-xl p-6 border border-gray-800">
                            ...
                            <span className="text-gray-400 text-sm">
                              ...
                              {index === 0
                                ? tokenXBalance.isLoading
                                  ? "Loading..."
                                  : Number(tokenXBalance.balance).toFixed(5)
                                : tokenYBalance.isLoading
                                  ? "Loading..."
                                  : Number(tokenYBalance.balance).toFixed(5)}
                            </span>
                            {token === "S" && sonicPrice && (
                          ...
                          <div className="flex items-center p-4">
                            ...
                            <div className="flex flex-col space-y-1">
                              ...
                              <button
                                ...
                                onClick={() =>
                                  handlePercentageClick(token, 100)
                                }
                              >
                                MAX
                              </button>
                        ...
                        <div className="flex items-center p-4">
                          ...
                          <div className="flex flex-col space-y-1">
                            <button
                              ...
                              onClick={() =>
                                handlePercentageClick("shares", 50)
                              }
                            >
                              50%
                            </button>
                            <button
                              className="px-3 py-1 bg-green-500 hover:bg-green-400 text-black text-xs font-medium rounded-lg transition-all duration-200 hover:shadow-md transform hover:scale-105"
                              onClick={() =>
                                handlePercentageClick("shares", 100)
                              }
                            >
                              MAX
                            </button>
                ...
                <div className="mt-8 space-y-4">
                  {activeTab === "deposit" ? (
                    (() => {
                      const token1Amount =
                        depositAmounts[vault.tokens[1]] || "0";
                      ...
                      const hasValidAmount =
                        !isNaN(tokenAmountFloat) && tokenAmountFloat >= 0.1;
      ...
      {/* Mobile Layout */}
      <div
        className={`sm:hidden mx-4 mb-4 ${isExpanded ? "vault-card-glow" : "vault-card"}`}
      >
        <div
                  ...
                  (() => {
                    ...
                    if (hasValidAmount && isUSDC) {
                      try {
                        tokenYAmount = parseUnits(token1Amount, 6);
                        needsApproval =
                          tokenYAmount > 0n && currentAllowance < tokenYAmount;
                      } catch (error) {
                        console.error("Error parsing token amount:", error);
                      }
                    }
