...
function PositionRangeIndicator({ vaultName }: PositionRangeIndicatorProps) {
  const { data: positionData, isLoading, activeId, price } = useMetroPositionData(vaultName);
  ...
  console.log('📊 Position Range Indicator:', {
    ...
    positionStatus: positionData.isInRange ? 'ACTIVE (In Range)' : 'OUT OF RANGE',
    isInRange: positionData.isInRange,
    hasValidRange: positionData.lowerBin !== 0 || positionData.upperBin !== 0
  });

  const rangePercentage = positionData.lowerBin && positionData.upperBin && positionData.activeId
    ? ((positionData.activeId - positionData.lowerBin) / (positionData.upperBin - positionData.lowerBin)) * 100
    : 50;
  ...
  return (
    <div className="flex flex-col gap-1.5 p-2 bg-arca-surface border border-arca-border rounded-md">
      ...
      <div className="flex items-center justify-between">
        <div className="text-xs font-medium text-arca-secondary">Metro Position</div>
        <div
      ...
      <div className="grid grid-cols-3 gap-2 text-xs">
        <div className="flex flex-col">
          ...
          <span className="font-mono text-white font-medium">{positionData.activeId}</span>
        </div>
        <div className="flex flex-col">
          <span className="text-arca-secondary text-xs">Range</span>
          <span className="font-mono text-white font-medium text-xs">{positionData.lowerBin || 0} - {positionData.upperBin || 0}</span>
        </div>
        <div className="flex flex-col">
        ...
        <div className="space-y-0.5">
          ...
          <div className="w-full bg-arca-border rounded-full h-1">
            <div 
              ...
              style={{ width: `${Math.max(5, Math.min(95, rangePercentage))}%` }}
            />
          </div>
        </div>
      )}
