...
export function useTokenBalance(tokenAddress: string) {
  ...
  const {
    data: erc20Decimals,
    isLoading: erc20DecimalsLoading,
    error: erc20DecimalsError,
  } = useReadContract({
    address: tokenAddress as `0x${string}`,
    abi: ERC20_ABI,
  ...
  const isLoading = isNativeToken
    ? nativeLoading
    : erc20BalanceLoading || erc20DecimalsLoading;
  const error = isNativeToken
    ? nativeError
    : erc20BalanceError || erc20DecimalsError;
