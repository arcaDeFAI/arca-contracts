...
// Hook to fetch real-time Shadow vault TVL data
export function useShadowVaultTvl(vaultName: string) {
  // Get strategy contract address for Shadow vaults
  ...
  // Fetch vault balances from Shadow strategy contract using getBalances()
  const {
    data: balances,
  ...
  const calculateTvl = () => {
    if (!balances || !sonicPrice) {
      if (vaultName === "S/USDC-CL") {
        console.log("Shadow Farm TVL calculation failed:", {
          vaultName,
          strategyAddress,
    ...
    // Debug logging for Shadow S/USDC-CL Farm TVL
    if (vaultName === "S/USDC-CL") {
      console.log("Shadow Farm TVL calculation for S/USDC-CL:", {
        strategyAddress,
