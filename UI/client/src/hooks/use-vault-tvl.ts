...
export function useVaultTvl(vaultName: string) {
  ...
  const { data: balances, isLoading: balancesLoading, error: balancesError } = useReadContract({
  ...
  const { data: sonicPrice, isLoading: priceLoading, error: priceError } = useSonicPrice();
  ...
  const calculateTvl = () => {
    ...
    const tvl = (sonicAmount * sonicPrice) + (usdcAmount * 1.00); // USDC is always ~$1
