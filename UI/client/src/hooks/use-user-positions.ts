...
export function useUserPositions() {
  ...
  const vaultPositions = mockVaults.map(vault => {
    ...
    const vaultAddress = vault.name === 'S/USDC' 
      ? '0x9541962342A344569FEAD20F6f824856aAC8cad9' 
      : fetchedVaultAddress;
    const { totalDeposited: historicalDeposited } = useDepositHistory(vaultAddress || '');
    ...
    if (vault.name === 'S/USDC') {
      console.log(`balanceOf call for ${vault.name}:`, {
        ...
        previewAmounts: previewAmounts ? [previewAmounts[0].toString(), previewAmounts[1].toString()] : 'undefined',
  ...
  const activePositions = useMemo(() => {
    return vaultPositions
      ...
      .map(position => {
        ...
        const deposited = position.historicalDeposited > 0 ? position.historicalDeposited : currentValue * 0.85;
        ...
        if (position.vault.name === 'S/USDC') {
          console.log(`Position calculation for ${position.vault.name}:`, {
            ...
            previewAmounts: position.previewAmounts.map(amt => amt.toString()),
            amountX,
            amountY,
  ...
  const totalDeposited = activePositions.reduce((sum, pos) => sum + pos.deposited, 0);
  const totalEarnings = activePositions.reduce((sum, pos) => sum + pos.earnings, 0);
