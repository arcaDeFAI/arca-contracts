...
export default function Vaults() {
  ...
  const filteredAndSortedVaults = useMemo(() => {
    let filtered = mockVaults.filter(vault => {
      ...
      // Search filter
      if (filters.search && !vault.name.toLowerCase().includes(filters.search.toLowerCase())) {
        return false;
      }
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
      ...
      <div className="mb-6 sm:mb-8 flex items-center space-x-3 sm:space-x-4">
        <div className="w-10 sm:w-12 h-10 sm:h-12 flex items-center justify-center text-[#469c46]">
          <svg viewBox="0 0 24 24" className="w-8 sm:w-10 h-8 sm:h-10 text-arca-primary" fill="currentColor" stroke="currentColor" strokeWidth="1.5">
        ...
        <div className="flex flex-col gap-4 w-full">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-start sm:items-center">
            ...
            <div className="flex items-center space-x-2 w-full sm:w-auto">
              <span className="text-arca-secondary font-medium text-xs sm:text-sm whitespace-nowrap">PLATFORM:</span>
              <Select value={filters.platform} onValueChange={(value) => setFilters(prev => ({ ...prev, platform: value }))}>
                ...
                <SelectContent className="bg-arca-surface border-arca-border">
                  {platforms.map(platform => (
                    <SelectItem key={platform} value={platform} className="text-white hover:bg-arca-border text-sm">
                      {platform}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            {/* Chain Filter */}
            <div className="flex items-center space-x-2 w-full sm:w-auto">
              <span className="text-arca-secondary font-medium text-xs sm:text-sm whitespace-nowrap">CHAIN:</span>
              <Select value={filters.chain} onValueChange={(value) => setFilters(prev => ({ ...prev, chain: value }))}>
                ...
                <SelectContent className="bg-arca-surface border-arca-border">
                  {chains.map(chain => (
                    <SelectItem key={chain} value={chain} className="text-white hover:bg-arca-border text-sm">
                      {chain}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            {/* Sort Filter */}
            <div className="flex items-center space-x-2 w-full sm:w-auto">
              <span className="text-arca-secondary font-medium text-xs sm:text-sm whitespace-nowrap">SORT BY:</span>
              <Select value={filters.sortBy} onValueChange={(value) => setFilters(prev => ({ ...prev, sortBy: value }))}>
                ...
                <SelectContent className="bg-arca-surface border-arca-border">
                  {sortOptions.map(option => (
                    <SelectItem key={option} value={option} className="text-white hover:bg-arca-border text-sm">
                      {option}
                    </SelectItem>
                  ))}
                </SelectContent>
      ...
      <div className="overflow-hidden">
        ...
        <div className="hidden sm:grid grid-cols-5 gap-6 px-10 py-4 mb-4">
          ...
          <div className="text-arca-secondary font-medium text-sm">EARNINGS</div>
          <div className="text-arca-secondary font-medium text-sm">POOL TVL</div>
          <div className="text-arca-secondary font-medium text-sm">FARM TVL</div>
        ...
        {filteredAndSortedVaults.length > 0 ? (
          ...
          <div className="px-6 py-12 text-center bg-arca-surface rounded-xl border border-arca-border mx-4">
            <p className="text-arca-secondary">No vaults found matching your criteria.</p>
          </div>
        )}
      </div>
    </main>
