...
export default function Dashboard() {
  ...
  const { activePositions, totalDeposited, totalEarnings, totalBalance, isLoading } = useUserPositions();
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="arca-primary font-medium text-lg">
                  {isLoading ? 'Loading...' : `+${formatCurrency(totalEarnings)}`}
                </span>
              </div>
      ...
      <div className="bg-arca-surface rounded-xl border border-arca-border p-6 sm:p-8">
        ...
        {isLoading ? (
            ...
            {activePositions.map((position, index) => (
              ...
              >
                <div className="flex items-center space-x-2 mb-3">
                  ...
                  <div>
                    <div className="text-white font-medium">{position.vaultName}</div>
                    <div className="text-arca-secondary text-xs">{position.platform}</div>
                  </div>
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span className="text-arca-secondary text-sm">Deposited:</span>
                    <span className="text-white text-sm">{formatCurrency(position.deposited)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-arca-secondary text-sm">Current Value:</span>
                    <span className="text-white text-sm">{formatCurrency(position.currentValue)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-arca-secondary text-sm">Earnings:</span>
                    <span className={`text-sm ${position.earnings >= 0 ? 'arca-primary' : 'text-red-400'}`}>
                      {position.earnings >= 0 ? '+' : ''}{formatCurrency(position.earnings)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-arca-secondary text-sm">APR:</span>
                    <span className="arca-primary text-sm font-medium">{position.apr}%</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="text-center py-8">
            ...
            <div className="text-arca-secondary text-sm">Start by depositing into a vault to see your positions here</div>
          </div>
        )}
      </div>
    </main>
