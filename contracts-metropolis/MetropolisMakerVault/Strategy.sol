...
import {SafeCast} from "../../lib/joe-v2/src/libraries/math/SafeCast.sol";
import {PriceHelper} from "../../lib/joe-v2/src/libraries/PriceHelper.sol";
import {IERC20Upgradeable} from "./libraries/joe-v2-periphery/IERC20Upgradeable.sol";
import {ILBPair} from "../../lib/joe-v2/src/interfaces/ILBPair.sol";
import {ILBToken} from "../../lib/joe-v2/src/interfaces/ILBToken.sol";
import {LiquidityAmounts} from "./libraries/joe-v2-periphery/LiquidityAmounts.sol";
...
import {Clone} from "../../lib/joe-v2/src/libraries/Clone.sol";
import {ReentrancyGuardUpgradeable} from "../../lib/joe-v2/lib/openzeppelin-contracts-upgradeable/contracts/utils/ReentrancyGuardUpgradeable.sol";
import {SafeERC20Upgradeable} from "./libraries/joe-v2-periphery/SafeERC20Upgradeable.sol";
...
import {console} from "../../lib/joe-v2/lib/forge-std/src/console.sol";
/**
...
contract Strategy is Clone, ReentrancyGuardUpgradeable, IStrategy {
    ...
    uint256 private _rebalanceCoolDown; // cool down period after rebalance
    ...
    function _checkPrice() internal view {
        ...
        IOracleVault(_vault()).checkPriceInDeviation();
    }
    ...
    /**
     * @notice Sets the rebalance cool down.
     * @dev Only the factory can call this function.
    ...
    function _harvestRewards() internal {
        ...
        // get the reward token and extra reward token
        IERC20 rewardToken = rewarder.getRewardToken();
        IHooksRewarder extraRewarder = _getExtraRewarder();
        IERC20 extraToken;
        bool hasExtra = false;
        if (address(extraRewarder) != address(0)) {
            extraToken = extraRewarder.getRewardToken();
            hasExtra = true;
        }
        ...
        // notify the vault
        _notifyVault(rewardToken);

        TokenHelper.safeTransfer(rewardToken, _vault(), rewardAmount);
        ...
        if (hasExtra) {
            ...
            if (extraAmount > 0) {
                _notifyVault(extraToken);
                TokenHelper.safeTransfer(extraToken, _vault(), extraAmount);
            }
        }
