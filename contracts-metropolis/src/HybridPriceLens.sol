...
contract HybridPriceLens is IPriceLens, Ownable {
    ...
    function _getRoutedLBPrice(
        ...
        address tokenX = address(pair.getTokenX());
        address tokenY = address(pair.getTokenY());
        if (feed.isTokenX && tokenX != _token)
            revert PriceLens__InvalidLBPair();
        if (!feed.isTokenX && tokenY != _token)
            revert PriceLens__InvalidLBPair();
        ...
        uint256 referencePriceInNative = _getTokenPriceNativeInternal(
            feed.referenceToken
        );
     ...
     */
    function _getTokenPriceNativeInternal(
        address _token
    ...
    function setExternalOracle(
        ...
        if (token == address(0) || oracleFeed == address(0))
            revert PriceLens__InvalidAddress();
    ...
    function setLBPairRoute(
        ...
        if (token == address(0) || lbPair == address(0))
            revert PriceLens__InvalidAddress();
        ...
        if (referenceToken == address(0)) {
            // Direct pair with native
            validPair =
                (isTokenX && token == tokenX && wnative == tokenY) ||
                (!isTokenX && token == tokenY && wnative == tokenX);
        } else {
            // Pair with reference token
            if (!isReferenceToken[referenceToken])
                revert PriceLens__InvalidRoute();
            validPair =
                (isTokenX && token == tokenX && referenceToken == tokenY) ||
